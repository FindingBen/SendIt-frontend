...
export const MessageCard = ({
  ...
  return (
    <div
      className={`${
        activeCampaigns || sceduledCampaigns || draftCampaigns
          ? "grid grid-cols-4"
          : "grid grid-cols-5"
    ...
    >
      ...
      <div>
        {message.status === "Draft" ? (
          <span class="text-xs 2xl:text-normal font-medium leading-none text-center text-white bg-green-600 rounded-lg px-3 lg:py-1">
            Draft
          </span>
        ) : message.status === "Scheduled" ? (
      ...
      {!activeCampaigns ? (
        <>
          {sceduledCampaigns ? (
            <button
              onClick={callmodal}
              className="border-gray-700 border-2 hover:bg-gray-600 hover:cursor-pointer px-2 py-1 rounded-md mx-auto text-sm xl:text-normal"
            >
              Cancel
            </button>
          ) : (
