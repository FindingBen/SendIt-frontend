...
const GenerateModal = ({ showModal, onClose, option, generateConfirm }) => {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      await generateConfirm(); // Make sure sendConfirm returns a Promise!
      setLoading(false);
      closeModal();
    } catch (err) {
      setLoading(false);
              ...
              <div className="relative flex flex-col rounded-xl w-full bg-ngrokGray">
                ...
                <div className="relative p-6 flex-auto">
                  <p className="my-4 text-slate-500 text-lg leading-relaxed">
                    You are about to generate <span className="font-bold">{option}</span> for your selected products. Are you sure?
                  </p>
                  {errorMsg && (
