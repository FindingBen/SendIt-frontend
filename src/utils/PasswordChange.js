...
const PasswordChange = ({ status, errStatus }) => {
  ...
  return (
    <div className="flex-1 px-1">
      ...
      {!isLoading ? (
        <button
          className="bg-sky-800 hover:bg-sky-400 mt-3 text-white text-sm xl:text-base font-light py-1 px-2 xl:py-2 xl:px-4 rounded"
          onClick={passChange}
        >
          Change
        </button>
      ) : (
        <button className="bg-sky-800 hover:bg-sky-400 mt-3 text-white text-sm xl:text-base font-light py-1 px-2 xl:py-2 xl:px-4 rounded flex flex-row">
          <svg
            aria-hidden="true"
          ...
          <span className="text-sm mt-0.5">Changing..</span>
        </button>
      )}
    </div>
  );
